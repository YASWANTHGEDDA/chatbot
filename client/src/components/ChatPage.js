...
// ===================================================================================
//  UI Sub-Components for the New Layout
// ===================================================================================

const ActivityBar = ({ activeView, setActiveView }) => (
    <div className="activity-bar">
        <button
            className={`activity-button ${activeView === 'ASSISTANT' ? 'active' : ''}`}
        ...
        <button
            className={`activity-button ${activeView === 'DATA' ? 'active' : ''}`}
...
const AssistantSettingsPanel = (props) => (
    <div className="sidebar-panel">
        ...
        <div className="llm-settings-widget">
            ...
            {LLM_OPTIONS[props.llmProvider] && LLM_OPTIONS[props.llmProvider].models.length > 0 && (
                <div className="setting-item">
...
// ===================================================================================
//  Main ChatPage Component (Now acts as Layout Orchestrator)
// ===================================================================================
...
const ChatPage = ({ setIsAuthenticated }) => {
    ...
    // START of LOGIC and HANDLER functions
    ...
    const saveAndReset = useCallback(async (isLoggingOut = false, onCompleteCallback = null) => {
        ...
        try {
            ...
            if (!newSessionIdForStorage) { throw new Error("Backend failed to provide new session ID for storage."); }
            localStorage.setItem('sessionId', newSessionIdForStorage);
            setSessionId(newSessionIdForStorage);
            setMessages([]);
            if (!isLoggingOut) {
                // **FIX**: Corrected the typo here from `handlePromptSelectchange` to `handlePromptSelectChange`.
    ...
    }, [messages, isLoading, handlePromptSelectChange]); // **FIX**: Added `handlePromptSelectChange` to dependency array.
    ...
    const handleNewChat = useCallback(() => { if (!isLoading) saveAndReset(false, null); }, [isLoading, saveAndReset]);
    
    const handleSendMessage = useCallback(async (e) => { if (e) e.preventDefault(); const textToSend = inputText.trim(); const currentSessionIdFromStorage = localStorage.getItem('sessionId'); const currentUserIdFromStorage = localStorage.getItem('userId'); if (!textToSend) { setError("Message text cannot be empty."); return; } if (isLoading) return; if (!currentSessionIdFromStorage || !currentUserIdFromStorage) { setError("Critical Error: Session or User ID missing. Please try logging out and in again."); return; } const newUserMessage = { role: 'user', parts: [{ text: textToSend }], timestamp: new Date().toISOString(), }; setMessages(prev => [...prev, newUserMessage]); setInputText(''); setError(''); setIsLoading(true); const historyForBackend = messages.map(m => ({ role: m.role, parts: m.parts, })); const messageData = { message: textToSend, history: historyForBackend, sessionId: currentSessionIdFromStorage, systemPrompt: editableSystemPromptText, isRagEnabled: isRagEnabled, llmProvider: llmProvider, llmModelName: llmModelName || null, enableMultiQuery: enableMultiQuery, }; try { const response = await sendMessage(messageData); const responseData = response.data; if (!responseData || !responseData.reply || !responseData.reply.parts || !responseData.reply.parts[0]) { throw new Error("Received an invalid or incomplete response from the AI service."); } const botMessage = { role: 'model', parts: responseData.reply.parts, references: responseData.reply.references || [], thinking: responseData.reply.thinking || null, timestamp: responseData.reply.timestamp || new Date().toISOString() }; setMessages(prev => [...prev, botMessage]); } catch (err) { const errorMessage = err.response?.data?.message || err.message || 'Failed to get response.'; setError(`Chat Error: ${errorMessage}`); const systemErrorMessage = { role: 'model', parts: [{ text: `Error: ${errorMessage}` }], timestamp: new Date().toISOString(), isError: true }; setMessages(prev => [...prev, systemErrorMessage]); } finally { setIsLoading(false); } }, [inputText, isLoading, messages, editableSystemPromptText, isRagEnabled, llmProvider, llmModelName, enableMultiQuery, navigate]);
    ...
    if (!localStorage.getItem('userId') && window.location.pathname !== '/login') { return <div className="loading-indicator"><span>Session data missing. Redirecting...</span></div>; }
    // END of LOGIC and HANDLER functions

    const isProcessing = isLoading;
    const sidebarProps = {
        currentSystemPromptId, editableSystemPromptText, handlePromptSelectChange, handlePromptTextChange,
        llmProvider, llmModelName, handleLlmProviderChange, handleLlmModelChange, isProcessing,
        enableMultiQuery, handleMultiQueryToggle, isRagEnabled, triggerFileRefresh,
        refreshTrigger: fileRefreshTrigger, onAnalysisComplete: handleAnalysisComplete
    };
        ...
        <div className="main-layout">
            ...
            <div className="chat-view">
                ...
                <footer className="input-area">
                    <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={handleEnterKey} placeholder="Type your message..." rows="1" disabled={isProcessing} />
