...
router.post('/', protect, upload.single('file'), async (req, res) => {
    ...
    if (!req.user) {
        console.error("!!! Upload handler: req.user not found after auth middleware.");
    ...
    if (req.fileError) {
        console.error(`!!! Error during upload middleware for user ${req.user.username}:`, req.fileError);
        if (req.fileError instanceof multer.MulterError) {
            let message = "File upload failed.";
            if (req.fileError.code === 'LIMIT_FILE_SIZE') message = `File too large. Max: ${Math.round(MAX_FILE_SIZE / 1024 / 1024)} MB.`;
            else if (req.fileError.code === 'LIMIT_UNEXPECTED_FILE') message = req.fileError.message || 'Invalid file type.';
            else if (req.fileError.code === 'UNAUTHENTICATED') message = req.fileError.message || 'Authentication required.';
