...
//             // Extract token, username, and _id from the response data
//             const { token, username: loggedInUsername, _id: userId } = response.data;

//             // Validate that we received the necessary data
//             if (!userId || !token || !loggedInUsername) {
//                  console.error("Incomplete data received from server:", { userId, token, loggedInUsername });
...
//             console.log("Auth data stored in localStorage:", { token: localStorage.getItem('token'), userId: localStorage.getItem('userId'), username: localStorage.getItem('username') });
...
const AuthPage = ({ setIsAuthenticated }) => {
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            // Explicitly remove any old sessionId to ensure ChatPage generates a new one
            localStorage.removeItem('sessionId');
            console.log("AuthPage: Cleared previous sessionId (if any) from localStorage upon successful auth.");
            ...
            localStorage.removeItem('username');
            localStorage.removeItem('userId');
            localStorage.removeItem('sessionId'); // Also clear sessionId on error
            setIsAuthenticated(false);
        } finally {
            setLoading(false);
        }
