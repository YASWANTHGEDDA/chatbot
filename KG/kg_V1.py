...
file_path = "./default_pdfs/229.txt"
with open(file_path, "r", encoding="utf-8") as file:  # FIXED: UnicodeDecodeError
    lecture_notes = file.read()
...
def merge_graphs(graphs):
    ...
    for i, graph in enumerate(graphs):
        ...
        for edge in graph.get('edges', []):
            if not isinstance(edge, dict):
                continue
            f, t, rel = edge.get("from"), edge.get("to"), edge.get("relationship")
            if isinstance(f, str) and isinstance(t, str) and isinstance(rel, str):
                final_edges.add((f, t, rel))

    final_edge_list = [{"from": e[0], "to": e[1], "relationship": e[2]} for e in final_edges]
    return {"nodes": list(final_nodes.values()), "edges": final_edge_list}
...
# Split the lecture notes
chunks = split_into_chunks(lecture_notes)
...
for i, chunk in enumerate(tqdm(chunks, desc="Processing chunks", unit="chunk")):
    ...
    try:
        ...
        if isinstance(graph_data, dict) and "nodes" in graph_data and "edges" in graph_data:
            ...
            logger.warning(f"Invalid graph structure in chunk {i+1}: {content[:200]}")

    except json.JSONDecodeError as e:
        logger.error(f"JSON error in chunk {i+1}: {e}")
    except Exception as e:
...
if all_graphs:
    final_graph = merge_graphs(all_graphs)
    logger.info(f"Merged {len(final_graph['nodes'])} nodes and {len(final_graph['edges'])} edges.")
    save_graph(final_graph)
else:
    logger.error("No valid graphs to merge.")
